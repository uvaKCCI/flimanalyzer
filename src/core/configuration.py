...
class Config():
            ...
            if parentconfig is None or not isinstance(parentconfig, dict):
                ...
                if addmissing:
                    ...
                    for key in parentkeys:
                        if not params.has_key(key) or params[key] is None:
    ...
    def get_parent(self, searchkey, parameters=None, returnkeys=False):
        ...
        if isinstance(searchkey, collections.Iterable) and not isinstance(searchkey, str) and not isinstance(searchkey, unicode) and len(searchkey) > 1:            
        ...
        if not parentconfig or not parentconfig.has_key(lastsearchkey):
            parentconfig = None
            parentkeys = []
        if returnkeys:
    ...
    def get(self, searchkey=None, startin=None, returnkeys=False):
        ...
        if isinstance(startin, dict):
            if isinstance(searchkey, collections.Iterable) and not isinstance(searchkey, str) and not isinstance(searchkey, unicode):
                if len(searchkey) == 0:
...
#                print "found ", searchkey, "in", keys  
    ...
    def validate(self):
        ...
        for longkey in self._get_required_keys():
            ...
            if parentdict is None or not parentdict.has_key(longkey[-1]):
        ...
        parameters = {
            ...
            CONFIG_ANALYSIS: {
                CONFIG_HISTOGRAMS: {
                    ...
                    'NAD(P)H t1': [0,800,51,['Treatment']],
                    'NAD(P)H t2': [0,400,51,['Treatment']],
                    'NAD(P)H tm': [0,2000,41,['Treatment']],
                    'NAD(P)H photons': [0,2000,41,['Treatment']],
                    'NAD(P)H a2[%]': [0,2000,41,['Treatment']],
                  #  'NAD(P)H a2[%]': [0,100,51,['Treatment']],
        #                    'NAD(P)H %': [0,99,34,['Treatment']],
                    'NADPH %': [0,99,34,['Treatment']],
                    'NADH %': [0,100,51,['Treatment']],
                    'NAD(P)H/NADH': [0,3,41,['Treatment']],
                    'NAD(P)H chi': [0.7,4.7,31,['Treatment']],
                    'NAD(P)H offset': [0.7,4.7,31,['Treatment']],
                    'NAD(P)H scatter': [0.7,4.7,31,['Treatment']],
                    'FAD t1': [0,4000,51,['Treatment']],
                    'FAD t2': [1000,7000,51,['Treatment']],
                    'FAD tm': [0,4000,51,['Treatment']],
                    'FAD a1[%]': [0,100,51,['Treatment']],
                    'FAD a2[%]': [0,100,21,['Treatment']],
                    'FAD a1[%]/a2[%]': [0,16,81,['Treatment']],
                    'FAD chi': [0.7,4.7,31,['Treatment']],
                    'FAD photons': [0,800,51,['Treatment']],
                    'FAD offset': [0,15,31,['Treatment']],
                    'FAD scatter': [0,15,31,['Treatment']],
                    'FLIRR': [0,2.4,41,['Treatment']],
