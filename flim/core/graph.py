...
class WorkflowGraph():
    ...
    def create_graph(self, state=None):
        ...
        self.obj_ids = {id(tr):(id(list(tr.input.values())[0]) if isinstance(tr, DataBucket) else id(tr)) for tr in task_refs}
    ...
    def get_plot(self, ax=None, fontsize=16, callback=None):
        ...
        ecs = {node_id:(1., 0.5, 0.5) if node_id in self.output_nodes else (0.5, 0.5, 1.0) for node_id in pos.keys()}
        fcs = {node_id:(1., 0.8, 0.8) if node_id in self.output_nodes else (0.8, 0.8, 1.0) for node_id in pos.keys()}
        ...
        for node_id, (x,y) in pos.items():
            ...
            tp = TextPath((originx,originy), self.node_labels[node_id], size=fontsize, prop=fp)
            ax.set_aspect(1.0)
