...
class dataanalyzer:
    ...
    def apply_filter(
        self, data, dropna=True, onlyselected=True, inplace=True, dropsonly=False
    ):
        ...
        if data is None:
            return (
                None,
                usedfilters,
                [[k, self.rangefilters[k]] for k in self.rangefilters],
                len(alldroppedrows),
            )
        logging.debug(
            "dataanalyzer.apply_filter: filtering %d rows, %d filters, onlyselected=%s"
            % (data.shape[0], len(self.rangefilters), str(onlyselected))
        )
        # if dropna:
        ...
        for seriesname in self.seriesfilter:
            logging.debug(
                f"Required series: {seriesname}, {self.seriesfilter[seriesname]}"
            )
            ...
            pdata = sdata.pivot_table(
                index=indexgroups, columns=[seriesname], aggfunc="count"
            )
            # restrict to seriesfilter columns of interest
            pdata = pdata.loc[
                :, pdata.columns.get_level_values(1).isin(self.seriesfilter[seriesname])
            ]
            logging.debug(pdata)
            ...
            if not droppedindex.empty:
                filtereddata = filtereddata.set_index(
                    droppedindex.names, drop=False
                ).drop(droppedindex)
    ...
    def categorize_data(
        ...
        med = (
            data.groupby(grouping)[col].median().unstack(unstack_levels)
        ...
        med = pd.merge(
            med.rename(columns=str).reset_index(),
            rel.rename(columns=str).reset_index(),
            on=non_ref_cols,
        )
        if joinmaster:
            ...
            joineddata = data.join(rel[category_colheader], on=non_ref_cols).rename(
                index=str, columns={category_colheader: category_colheader}
            )
            logging.debug(joineddata.dtypes)
            if add_ascategory:
                joineddata[category_colheader].astype("category")
                unassigned = "unassigned"
                joineddata[category_colheader] = joineddata[
                    category_colheader
                ].cat.add_categories([unassigned])
                logging.debug(
                    f"CATEGORIES={joineddata[category_colheader].cat.categories}"
                )
