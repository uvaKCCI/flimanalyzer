...
class Config:
                ...
                if addmissing:
                    ...
                    for key in parentkeys:
                        if not key in params or params[key] is None:
                            logging.debug(
                                f"\tcreating missing key {key} for {parentkeys}"
                            )
                            params[key] = {}
                        params = params[key]
                    parentconfig = params
    ...
    def get_parent(self, searchkey, parameters=None, returnkeys=False):
        ...
        if (
            isinstance(searchkey, collections.abc.Iterable)
            and not isinstance(searchkey, str)
            and not isinstance(searchkey, bytes)
            and len(searchkey) > 1
        ):
                ...
                if searchkey in startin:
                    ...
                    for key in startin:
                        ...
                        if subparams is not None:
                            subconfig, keys = self.get(
                                searchkey, startin=subparams, returnkeys=True
                            )
    ...
    def _to_utf(self, data, ignore_dicts=False):
        ...
        if isinstance(data, dict) and not ignore_dicts:
            return {
                self._to_utf(key, ignore_dicts=True): self._to_utf(
    ...
    def _get_required_keys(self):
        return (
            (CONFIG_ROOT, CONFIG_IMPORT, CONFIG_EXCLUDE_FILES),
    ...
    def fix(self):
        ...
        for longkey in set(missing + invalid):
            ...
            if params is not None:
                ...
                updated, notfound = config.update(
                    {longkey[-1]: default.get(longkey)}, longkey[:-1]
                )
            for u in updated:
    ...
    def create_default(self):
        ...
        datadisplay = [
            {"name": aname, "min": "auto", "max": "auto", "bins": 100}
            for aname in ["trp t1", "trp t2", "trp tm"]
        ]
        plugins_config = init_plugins_configs()

        parameters = {
            CONFIG_WORKINGDIR: os.path.join(
            ...
            CONFIG_FILTERS: {
                ...
                CONFIG_RANGEFILTERS: [
                    ...
                    RangeFilter("NAD(P)H a1", 0, 1000).get_params(),
                    RangeFilter("NAD(P)H a1[%]", 0, 100).get_params(),
                    RangeFilter("NAD(P)H a2", 5, 300).get_params(),
                    RangeFilter("NAD(P)H a2[%]", 0, 100).get_params(),
                    RangeFilter("NAD(P)H t1", 10, 1000).get_params(),
                    RangeFilter("NAD(P)H t2", 2000, 7000).get_params(),
                    RangeFilter("NAD(P)H tm", 0, 2000).get_params(),
                    RangeFilter("NAD(P)H photons", 10, 2000).get_params(),
                    RangeFilter("NAD(P)H a2[%]", 0, 100).get_params(),
                    ...
                    RangeFilter("FAD t1", 10, 1500).get_params(),
                    RangeFilter("FAD t2", 1000, 6000).get_params(),
                    RangeFilter("FAD tm", 0, 2500).get_params(),
                    RangeFilter("FAD a1", 10, 500).get_params(),
                    RangeFilter("FAD a1[%]", 0, 100).get_params(),
                    RangeFilter("FAD a2", 10, 300).get_params(),
