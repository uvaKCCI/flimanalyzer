...
class UnPivotConfigDlg(BasicAnalysisConfigDlg):
    ...
    def __init__(self, parent, title, input=None, 
        ...
        BasicAnalysisConfigDlg.__init__(self, parent, title, input=input, enablegrouping=False, enablefeatures=True, selectedgrouping=selectedgrouping, selectedfeatures=selectedfeatures, optgridrows=2, optgridcols=1)
		    
    def get_option_panels(self):
        hsizer = wx.BoxSizer(wx.VERTICAL)
        self.category_field = wx.TextCtrl(self.panel, value=self.category_name, size=(500,-1))
        hsizer.Add(wx.StaticText(self.panel, label='New Category Column'), 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5)
        hsizer.Add(self.category_field, 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5)
                
        self.feature_field = wx.TextCtrl(self.panel, value=self.feature_name, size=(500,-1))
        hsizer.Add(wx.StaticText(self.panel, label='New Feature Column'), 0, wx.ALL|wx.ALIGN_CENTER_VERTICAL, 5)
...
class UnPivot(AbstractPlugin):
    ...
    def run_configuration_dialog(self, parent, data_choices={}):
        ...
        dlg = UnPivotConfigDlg(parent, 'Unpivot Data', 
            input=self.input, 
    ...
    def execute(self):
        ...
        new_cat_name = self._create_unique_name(self.params['category_name'], 'Category', existing=cat_cols)
        new_value_name = self._create_unique_name(self.params['feature_name'], 'Feature', existing=sel_features)
