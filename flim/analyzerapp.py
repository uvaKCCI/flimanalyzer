...
def parse_arguments():
    ...
    parser.add_argument(
        "-p",
        "--parser",
        default="core.parser.defaultparser",
        help="use this class to parse filename",
    )
    parser.add_argument(
        "-c", "--config", default=None, help="configuration file, superseeds -p option"
    )
    parser.add_argument(
        "-l",
        "--log",
...
def noninteractive_run(fa, args):
    ...
    pp.set_replacementheaders(
        {"Exc1_-Ch1-_": "trp ", "Exc1_-Ch2-_": "NAD(P)H ", "Exc2_-Ch3-_": "FAD "}
    )
    ...
    logging.debug(ch)
    logging.debug(
        "\nDropped %d columns: data contains %d rows, %d columns"
        % (len(dc), data.shape[0], data.shape[1])
    )
    logging.debug(dc)
